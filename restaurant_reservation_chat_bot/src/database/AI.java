...
public class AI {
		...
		do{
			...
			if(!userInput.equals("cancel")) {
				...
				for(int i = 0; i < ReservedTables.currentOrdinaryIndex; i++) {
					...
					if(ChatBot.table[0].get(i).guestName.equals(userInput)) {
						
						
						ChatBot.table[0].remove(i);
				...
				for(int i = 0; i < ReservedTables.currentVipIndex; i++) {
					if(ChatBot.table[1].get(i).guestName.equals(userInput)) {
						ChatBot.table[1].remove(i);
						System.out.println("RRCB: \nThe reservation has been removed.");
					} else {
	...
	public static void reserve() {
			...
			if(!userInput.equals("cancel")) {
				...
				if(sched.size() != 3) {
					...
					if(sched.get(2) < 2024) {
						...
						year = sched.get(2);
						System.out.println("\nRRBC: \nThank you for reserving to our restaurant!\n");
						break;
		...
		ReservedTables.book(ChatBot.table,j, guestName, numPeople, month, day, year, time, index);
